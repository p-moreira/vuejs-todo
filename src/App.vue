<template>
  <div id="app" :style="{margin: setMargin}">
    <template v-if="!hasName">
      <div class="fields">
        <label for="name" class="label">Qual o seu<br>nome?</label>
        <input type="text" id="name" placeholder="seu nome" class="input" v-model="name">
        <button class="btn" @click="saveName">Iniciar</button>
      </div>  
    </template>
    <template v-else>
      <Greeting :name="getName"></Greeting>
      <Tasks></Tasks>
    </template>
  </div>
</template>
...
.fields input:focus, .fields button:focus {
  outline: 0px;
}
.label {
  font-size: 40px;
